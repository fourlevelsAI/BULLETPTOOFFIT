...
import { Search, Plus, X, Clock, Camera, Mic, ScanLine } from "lucide-react";
...
import { localFoodDatabase, searchOpenFoodFacts, lookupBarcode, type FoodItem } from "@/lib/foodDatabase";
...
const MealsPage = () => {
  ...
  const addFood = async (food: FoodItem) => {
    ...
    else { toast.success(`${food.name} added to ${selectedMeal}`); setServings((p) => ({ ...p, [food.name]: 1 })); }
  };
        ...
        <motion.div variants={fadeUp}>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="section-label">
              {searchQuery ? (searching ? "Searching..." : "Search Results") : "Popular Foods"}
            </h2>
          </div>
          {displayedFoods.length === 0 ? (
            ...
            <div className="space-y-2">
              {displayedFoods.map((food) => {
                const mult = getServings(food.name);
                return (
                  <motion.div key={food.name + (food.brand || "")} whileHover={{ y: -2 }} className="bracket-card">
                    <div className="flex items-center justify-between">
                      <div className="flex-1 text-left min-w-0">
                        <span className="text-sm font-medium text-foreground font-body block truncate">{food.name}</span>
                      ...
                      <span className="text-sm font-semibold text-foreground font-mono ml-2 shrink-0">{Math.round(food.calories * mult)}</span>
                    </div>
